...
int main()
{
	...
	moveUnitMin = -5.f, moveUnitMax = 5.f;
	...
	while (window.isOpen())
	{
		while (window.pollEvent(event))
		{
			...
			if (event.type == Event::KeyPressed)
			{
				...
				if (event.key.code == Keyboard::Right)
				{
					...
					player.changePosture(60, 0, 60, 89, 1);
				}
				if (event.key.code == Keyboard::Left)
				{
					player.sprite.move(moveUnitMin, 0);
					player.changePosture(120, 0, 60, 89, 1);
				}
				if (event.key.code == Keyboard::Up)
				{
					player.sprite.move(0, moveUnitMin);
					player.changePosture(180, 0, 60, 89, 1);
				}
				if (event.key.code == Keyboard::Down)
				{
					player.sprite.move(0, moveUnitMax);
					player.changePosture(0, 0, 60, 89, 1);
				}
				//Weapon display controls A, D, W and S letter keys
				if (event.key.code == Keyboard::A)
				{
					player.changePosture(120, 90, 60, 89, 1);
				}
				if (event.key.code == Keyboard::D)
				{
					player.changePosture(60, 90, 60, 89, 1);
				}
				if (event.key.code == Keyboard::W)
				{
					player.changePosture(180, 90, 60, 89, 1);
				}
				if (event.key.code == Keyboard::S)
				{
					player.changePosture(0, 90, 60, 89, 1);
				}
			}
		}
		//Update
		...
		for (auto wall : walls)
			if (player.sprite.getGlobalBounds().intersects(wall.sprite.getGlobalBounds()))
			{
				cout <<"touched" << player.sprite.getGlobalBounds().left<<","<< player.sprite.getGlobalBounds().top\
				 <<" "<< player.sprite.getGlobalBounds().width<<","<< player.sprite.getGlobalBounds().height<<endl;
				//TODO
			}
		//Display all
